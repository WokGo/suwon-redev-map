...
const Home = () => {
  ...
  const assetBase = (process.env.PUBLIC_URL && process.env.PUBLIC_URL !== '.')
    ? process.env.PUBLIC_URL
    : (typeof window !== 'undefined' && window.location.pathname.startsWith('/admin') ? '/admin' : '');
  const heroImage = `${assetBase}/static/homeimg.jpg`;
  ...
  return (
    <div className="container mt-5 text-center" style={{ backgroundColor: '#add8e6', padding: '20px', borderRadius: '100px' }}>
      ...
      <img src={heroImage} alt="Welcome" className="img-fluid rounded my-3" />
